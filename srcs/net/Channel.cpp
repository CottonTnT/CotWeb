...
// // 这里充分体现了Channel只做事件的分发
// void Channel::HandleEventWithGuard_(Timestamp receiveTime)
// {
//     EASY_INFO("channel handleEvent revents:%d", received_events_);
//     // 关闭

//     // EPOLLHUP 表示对端完全关闭了连接, 如果同时有 POLLIN，说明还有数据没读完，本端先就不急着处理关闭。
//     // 优先读数据（因为有 POLLIN），等读完再"交给上层决定是否关闭"
